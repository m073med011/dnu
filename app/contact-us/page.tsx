...
const UniversityRegistrationForm: React.FC = () => {
  ...
  const validateTab = (tabId: number): FormErrors => {
    ...
    switch (tabId) {
      ...
      case 3:
        ...
        break;
      case 4:
        // Conditional validation for sibling fields
  ...
  const handleInputChange = (
    ...
    // Special handling for the checkbox to clear sibling fields when unchecked
    if (field === "hasTeachingExperience" && value === false) {
      ...
      // Clear errors for sibling fields
      setErrors((prev) => {
  ...
  const handleSubmit = async () => {
    ...
    const formattedData = {
      ...
      agree_terms: 1, // Assuming agreement when submitting
    };
    try {
      ...
      const response = await axios.post(
        ...
        formattedData,
        {
          headers: {
            ...
            // Note: withCredentials is typically a fetch option, for axios you might need to configure it differently
            // depending on CORS requirements (e.g., axios.defaults.withCredentials = true;)
          },
          // withCredentials: false, // This is a fetch option, not typically used directly in axios config like this
        }
      );
